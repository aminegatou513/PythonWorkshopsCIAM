...
def reverse_string1(str):
    ...
    for i in str:
        str1 = i + str1  # The for loop iterated every element of the given string, join each character in the
...
def reverse_string2(str):
    ...
    while count > 0:
        reverse_String += str[count - 1]  # save the value of str[count-1] in reverseString /// str[count-1] is the last character of
        # the string
...
def reverse_string4(str):
    ...
    if len(str) == 0:  # Checking the lenght of string
        ...
        return reverse_string4(str[1:]) + str[
            0]  # Recall the function inside itself with new parameter till the condition of len(str) == 0 is met
        '''
        Explanation (our string is abcd) /// first call : bcd and a is stored (since str[1:] will return the string without the element at index 0) len(str) is now 3 
                                             ...
                                             last call :   empty  and dcba is returned since len(str) is 0 now so the condition is met'''


def reverse_string5(str):
    # the reverse function doesn't reverse a string directly but returns: a reversed list of items present in a sequence
    # object. So to convert it to string we use the join method that transform the list into string a join it with an
    # empty string to have our reversed string
